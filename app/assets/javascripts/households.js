...
function formatHouseholdChore(householdChore) {
...
function formatHouseList(household) {
 		...
 		let id = $(this).data("id");
   		$.get("/households/" + id + ".json", function(data) {
   			...
   			for (let i = 0; i < household.chores.length; i++) {
				$("#household-" + id + "-chores").append(formatHouseholdChore(household.chores[i]));
			}
   		});
	})
	...
	$('#js-households').on('click', function (e) {
  		...
  		$.get("/households" +  ".json", function(data) {
  			...
  			for (let i = 0; i < data.length; i++) {
				...
				$("#households").append(formatHouseList(household));
			}
   		});
	})
